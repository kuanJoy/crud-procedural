        ...
        if ($password == $repassword) {
            ...
            if ($_FILES['photo']['error'] === 4) {
                ...
                if ($_FILES['photo']['error'] === 0) {
                    ...
                    $errors['photo_name'] = 'Неправильное имя фотографии';
                }
            }

            if (empty($errors)) {
                $insert_user = "INSERT INTO `users` (`id_user`, `name`, `surname`,`email`,`password`,`photo`,`id_group`, `id_role`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)";
                $params = ["sssssii", $user_name, $user_surname, $user_email, $hash_password, $img_upload_path, intval($id_group), intval($id_role)];
                $result_insert_user = executeQuery($connection, $insert_user, $params);
                if ($result_insert_user) {
                    $_POST['added'] = "Успешно добавлено";
                    header("Location: create.php?act=user");
                    exit();
                } else {
...
<form class="create" action="" method="post" enctype="multipart/form-data">
    ...
    <?php if (isset($_GET['year'])) { ?>
        ...
        <div class="photo-inputs">
            <div class="form_photo">
                ...
                <div class="photo__default">
                    <span>По умолчанию</span><input type="checkbox" name="users[photo]">
                </div>
            </div>
            <div class="form_rows">
                <div class="create_row">
                ...
                <div class="create_row">
                    <label class="mw-5" for="">Роль:</label>
                    <span>студент</span><input checked class="row_input" type="radio" name="users[role]" value="3">
                    <span>модератор</span><input class="row_input" type="radio" name="users[role]" value="2">
                </div>
            </div>
        </div>
        <?php
        show_errors();
        ?>
        <button class="my-btn" name="send" type="submit">Сохранить</button>
    <?php } ?>
</form>